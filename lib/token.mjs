...
const initSql = {
    ...
    [dbio.POSTGRESQL]: [[
        ...
        `CREATE INDEX IF NOT EXISTS ${table}_type_index ON ${table} (type)`,
    ], [
        ...
        `CREATE INDEX IF NOT EXISTS ${table}_expired_at_index ON ${table} (expired_at)`,
    ]],
};
...
const getLatestByUserAndType = async (user_id, type) => {
    let placeholder;
    switch (await dbio.getProvider()) {
        case dbio.MYSQL: placeholder = ['?', '?']; break;
        case dbio.POSTGRESQL: placeholder = ['$1', '$2']; break;
    }
    const sql = dbio.assembleQuery(table)
        + ` WHERE user_id = ${placeholder[0]} AND type = ${placeholder[1]}`
...
const verify = async (token, options) => {
    ...
    let ph1, ph2;
    switch (await dbio.getProvider()) {
        case dbio.MYSQL: ph1 = '?'; ph2 = '?'; break;
        case dbio.POSTGRESQL: ph1 = '$1'; ph2 = '$2'; break;
    }
    const sql = dbio.assembleQuery(table)
        + ` WHERE id = ${ph1} AND expired_at > ${ph2}`;
    const resp = (await dbio.query(sql, [token, 'NOW()']))[0];
...
const revokeByUserIdAndType = async (user_id, type, options) => {
    ...
    sql += ` WHERE ${dbio.rawAssembleKeyValue('user_id', user_id, { placeholderIndex: 2 })}`
        + ` AND ${dbio.rawAssembleKeyValue('type', type, { placeholderIndex: 3 })}`;
