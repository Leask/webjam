...
const validateUser = (data, options) => {
    ...
    switch (utilitas.trim(options.mode, { case: 'UP' })) {
        case 'CREATE':
            ...
            break;
        case 'UPDATE':
            utilitas.assert(options.curUser, errInvalid, 500);
            if (email
                && !utilitas.insensitiveCompare(options.curUser.email, email)) {
                utilitas.assertEmail(result.email = email);
...
const rawSendVerificationEmail = async (add, type, options) => {
    ...
    resp.type = utilitas.trim(type, { case: 'UP' });
...
const sendVerificationEmail = async (add, options) => {
    return await awSendVerificationEmail(add, typeEmail, options);
};

const sendPasswordRecoverEmail = async (add, options) => {
    return await awSendVerificationEmail(add, typePassword, options);
};

const create = async (data, options) => {
    data = validateUser(data, { mode: 'CREATE' });
    try {
        return packUser(await db.insert(table, data, options), options);
...
const updateById = async (id, data, options) => {
    ...
    if (!options.skipVerify) {
        data = validateUser(data,
            { mode: 'UPDATE', curUser: await queryById(id) });
    }
    const result = {};
    try {
        options.user = result.user = packUser(await db.updateById(
            table, id, data, options
        ), options);
    } catch (err) { handleDupEntryError(err); }
    if (data.email) {
        try {
            result.email = options.skipEmailVerify ? null
                : await sendVerificationEmail(result.user.email, options);
        } catch (err) { handle501Error(err); }
    }
    return result;
};

const changePasswordById = async (id, passwd, options) => {
    assertPassword(passwd);
    return await updateById(id, saltPassword(passwd), options);
};
...
const verifyEmail = async (strToken, options) => {
    ...
    const resp = await token.verifyForVerification(strToken, options);
    resp.user = await updateById(resp.userId, {
...
const changePasswordByVerificationToken = async (token, password) => {

};

const queryById = async (id, options) => {
    const resp = await db.queryById(table, id, options);
    assertUser(resp);
    return packUser(resp, options);
};

const queryByKeyValue = async (key, value, options) => {
    const resp = await db.queryByKeyValue(table, key, value, options);
...
const signup = async (data, options) => {
    options = options || {};
    const user = await create(data, options);
    const auth = await token.createForUser(user.id, options);
    options.user = user;
    let mRsp = null;
    try {
        mRsp = options.skipEmailVerify ? null
...
const { utilitas, encryption, db, email } = require('utilitas');
