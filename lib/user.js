...
const updateById = async (id, data, options) => {
    options = options || {};
    data = validateUser(data, { mode: 'UPDATE', curUser: await queryById(id) });
    let user = null;
    try {
        user = packUser(await db.updateById(
    ...
    return { user, email: mRsp };
};

const changePasswordById = async (id, passwd, options) => {
    assertId(id);
    assertPassword(passwd);
    return packUser(await db.updateById(
        table, id, saltPassword(passwd), options
    ), options);
};
