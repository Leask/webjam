...
import { cleanSql } from 'utilitas/lib/dbio.mjs';
...
const initSql = {
    ...
    [dbio.POSTGRESQL]: [[
        ...
        `CREATE UNIQUE INDEX IF NOT EXISTS email ON ${table} (email)`,
    ], [
        `CREATE INDEX IF NOT EXISTS password ON ${table} (password)`,
    ], [
        `CREATE INDEX IF NOT EXISTS salt ON ${table} (salt)`,
    ], [
        `CREATE INDEX IF NOT EXISTS avatar ON ${table} (avatar)`,
    ], [
        `CREATE INDEX IF NOT EXISTS bio ON ${table} (bio)`,
    ], [
        `CREATE INDEX IF NOT EXISTS email_verified_at ON ${table} (email_verified_at)`,
    ], [
...
const init = async (options) => {
    if (options) {
        ...
        for (const act of initSql[provider]) {
            _result.push(await dbio.query(...act));
        }
