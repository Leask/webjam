...
const query = async (_clase, userId, options) => {
    ...
    const [whereValCount, whereValQuery, status] = [[userId, 0], [userId, 0], {}];
    ...
    const [cResp, sResp, resp] = await Promise.all([[
        ...
        dbio.assembleQuery(table, { fields: ['COUNT("id")', 'status'] }) + whereKey + ' GROUP BY "status"',
        whereValCount,
    ], [
    ...
    sResp.map(x => status[x.status] = x['count']);
