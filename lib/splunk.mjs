...
import { assertToken } from './token.mjs';
...
const assertAuth = async (auth) =>
    assertToken(strToken && `Splunk ${strToken}` === auth);

const log = (content, host, options) => utilitas.log(
    content, utilitas.basename(import.meta.url) + (host ? ` > ${host}` : ''),
    { time: true, ...options || {} }
);
