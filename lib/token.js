...
const assertToken = (token) => {
    utilitas.assert(token, 'Invalid token.', 400);
};
...
const getLatestByUserAndType = async (userId, type) => {
    ...
    return database.query(sql, [user.assertId(userId), assertType(type)]);
};
...
const verify = async (token, type, options) => {
    ...
    const resp = database.query(sql, [token, type, new Date()]);
    if (resp && !options.skipUser) {
        resp.user = await user.getById(resp.userId, options);
    }
