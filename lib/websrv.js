...
const init = async (options) => {
    ...
    if (options.database) { database.init(options.database); }
...
const up = async (options, callback) => {
    ...
    log(`${(await utilitas.which()).title} is launching at ${new Date}...`);
    options = options || {};
    await init(options);
    ...
    log(`Successfully launched at ${global.websrv.origin} within `
        + `${(end[1] / 1000000 / 1000)} seconds.`);
    return global.websrv;
};
...
const { utilitas, database, email } = require('utilitas');
