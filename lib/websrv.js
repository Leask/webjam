...
const getSessionConfig = async (options) => {
    options = options || {};
    ...
    return {
        keys: utilitas.ensureArray(options.keys
            || encryption.sha256(JSON.stringify(config))), config,
    };
};
...
const init = async (options) => {
    ...
    const { keys, config: sesConfig } = await getSessionConfig(options.session);
    console.log(keys, sesConfig);
    websrv.app.keys = keys;
    websrv.app.use(session(sesConfig, websrv.app));
    ...
    if (options.thirdparties) { thirdParty.init(options.thirdparties); }
