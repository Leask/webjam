...
const init = async (options) => {
    ...
    if (options.tracing) { tracing.init(options.tracing, websrv.app); }
    ...
    if (options.googleCloud) { initGoogleCloud(options.googleCloud); }
...
const up = async (options, callback) => {
    ...
    if (options.tape) { tape.init(options.tape); }
    if (options.tracing) { tracing.init(options.tracing); }
