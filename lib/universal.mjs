...
const assertFunc = name => {
    const func = callosum.assertFunc(name);
    assert(func?.type === UNIVERSAL, 'Function is not universal.', 400);
    return func;
};

const getFunc = (name, options) => {
    let func = callosum.getFunc(name, options);
    if (name) { func = func?.type === UNIVERSAL ? func : null; } else {
        for (const name in func) {
            if (func[name]?.type !== UNIVERSAL) { delete func[name]; }
        }
    }
