...
const newUniversalStream = () => uoid.create({ type: 'STREAM_SESSION' });
...
const writeStreamSession = async (id, event, options) => {
    id || (id = newUniversalStream());
    const resp = await callosum.push(
        id, event, { ...options || {}, confirm: true, life }
    );
    return resp;
};
