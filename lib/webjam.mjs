...
import { init as initMeta } from './meta.mjs';
...
const toInit = {
    database: dbio.init, email: email.init, meta: initMeta, user: initUser,
};
...
const _init = async (options, callback) => {
    ...
    for (let i in toInit) {
        ...
        if (i === 'database' && !callosum.isPrimary && opts.vector) {
            opts.vector = false;
        }
        await toInit[i](opts);
    }
    await callosum.init({
        ...
        initWorker: async () => {
            ...
            const { key, cert } = await ssl.getCert();
            ...
            webjam.server = createServer({
                ...options.https ? { key, cert } : {}
            }, webjam.app.callback());
            webjam.service = webjam.server.listen(
                webjam.port, _ => Function.isFunction(callback) && callback(_)
            );
            process.on('SIGTERM', process.exit);
            process.on(ssl.SSL_RESET, () => {
                log(`Got ${ssl.SSL_RESET} signel, reloading...`);
                process.exit;
            });
        },
        onReady: async () => {
