...
import { utilitas, dbio, encryption, uoid } from 'utilitas';
...
const getPassport = async (provider = sDefault) => {
    if (!strategies[provider = assertProvider(provider)].module) {
        const { default: load } = await import(strategies[provider].package);
...
const authenticate = async (provider) => {
    return strategies[sDefault].module && await (await getPassport(
